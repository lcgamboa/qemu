...
uint32_t *qemu_picsimlab_get_internals(int cfg) {
  ...
  case QEMU_INTERNAL_UART0_BAUD:
    return &global_s->uart[0].baud_rate;
    break;
  case QEMU_INTERNAL_UART1_BAUD:
    return &global_s->uart[1].baud_rate;
    break;
  case QEMU_INTERNAL_UART2_BAUD:
    return &global_s->uart[2].baud_rate;
    break;
...
static void esp32_soc_realize(DeviceState *dev, Error **errp)
{
    ...
    object_property_set_bool(OBJECT(&s->spi[2]),"use_cs",true, &error_abort);
    object_property_set_bool(OBJECT(&s->spi[3]),"use_cs",true, &error_abort);
    for (int i = 0; i < ESP32_SPI_COUNT; ++i) {        
...
//ESP32-DevKitC V4
    ...
    if(pinmap){
      ...
      for(int pin = 1; pin < (pinmap[0]+1); pin++){
        if(pinmap[pin] >= 0 ){
           if(pinmap[pin]  < 32){
              qdev_connect_gpio_out_named(DEVICE(&s->gpio), ESP32_GPIOS, pinmap[pin], pout_irq[pin]);
              qdev_connect_gpio_out_named(DEVICE(&s->gpio), ESP32_GPIOS_DIR, pinmap[pin] , pdir_irq[pin]);
              pin_irq[pin]=qdev_get_gpio_in_named(DEVICE(&s->gpio), ESP32_GPIOS_IN, pinmap[pin]);
           }
           else{
              pin_irq[pin]=qdev_get_gpio_in_named(DEVICE(&s->gpio), ESP32_GPIOS_IN, pinmap[pin]);
           }
